...
<h2 class="sec">More head-to-heads</h2><div class="rel-grid" id="more-cmp"></div></div>"""
    ...
    # cross-link the head-to-heads into each page and build the index
    links = "".join(f'<a href="/compare/{s}/">{esc(t)}<small>{esc(w)}</small></a>' for s, t, w in made)
    for s, _, _ in made:
        p = SITE / "compare" / s / "index.html"
        p.write_text(p.read_text().replace('<div class="rel-grid" id="more-cmp"></div>',
                                           f'<div class="rel-grid">{links}</div>'))
    body = f"""<div class="hero lib-hero"><div class="wrap hero-inner">
